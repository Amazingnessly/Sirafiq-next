...
function pageStorageKey(identity: string) { return `${PDF_PAGE_STORAGE_PREFIX}${identity}`; }
function readSavedPage(identity: string, pageCount: number) {
  try {
    const value = Number.parseInt(window.localStorage.getItem(pageStorageKey(identity)) ?? '', 10);
    if (!Number.isFinite(value)) return 1;
    return Math.min(Math.max(value, 1), pageCount);
  } catch { return 1; }
}
function savePage(identity: string, pageNumber: number) {
...
export function PdfViewer({ src, title, storageId, onReadError }: PdfViewerProps) {
  ...
  useEffect(() => {
    ...
    const updateWidth = () => setStageWidth(node.clientWidth); updateWidth();
    if (typeof ResizeObserver !== 'undefined') { const observer = new ResizeObserver(updateWidth); observer.observe(node); return () => observer.disconnect(); }
    window.addEventListener('resize', updateWidth); return () => window.removeEventListener('resize', updateWidth);
  }, []);

  useEffect(() => {
    let cancelled = false; let loadingTask: PdfLoadingTask | null = null;
    setLoadingDocument(true); setError(null); setPageNumber(1); setPageCount(0);
    async function openDocument() {
      try {
        ...
        loadingTask = pdfjs.getDocument({ url: src, rangeChunkSize: 512 * 1024, disableStream: isServerDocument, disableAutoFetch: isServerDocument }) as unknown as PdfLoadingTask;
  ...
  useEffect(() => {
    const currentDocument = documentRef.current; const currentCanvas = canvasRef.current;
  ...
  return <section className="document-viewer pdf-reader" aria-label={`Lecteur PDF — ${title}`}>
    <div className="pdf-reader__toolbar">
      <button className="button button--secondary" type="button" disabled={loadingDocument || pageNumber <= 1} onClick={() => goToPage(pageNumber - 1)} aria-label="Page précédente">← Précédente</button>
      <div className="pdf-reader__position" aria-live="polite">{pageCount ? <label>Page{' '}<input type="number" min={1} max={pageCount} value={pageNumber} disabled={loadingDocument} onChange={(event) => goToPage(event.currentTarget.valueAsNumber)} aria-label="Aller à la page" inputMode="numeric" />{' '}sur {pageCount}</label> : 'Ouverture du PDF…'}</div>
      <button className="button button--secondary" type="button" disabled={loadingDocument || !pageCount || pageNumber >= pageCount} onClick={() => goToPage(pageNumber + 1)} aria-label="Page suivante">Suivante →</button>
    </div>
    <div className="pdf-reader__stage" ref={stageRef}>
      ...
      <canvas ref={canvasRef} aria-label={pageCount ? `Page ${pageNumber} du PDF ${title}` : `PDF ${title}`} className={loadingDocument || error ? 'is-hidden' : ''} />
