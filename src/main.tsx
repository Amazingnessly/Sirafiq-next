...
type Support = { id: string; name: string; type: string; size: number; importedAt: string; category?: string; blob?: Blob; dataUrl?: string; extraction?: Extraction; pdfProgress?: PdfProgress; pdfBookmarks?: number[]; pdfNotes?: PdfPageNote[]; flashcards?: Flashcard[]; recallDraft?: string; recallAttempts?: RecallAttempt[] };
...
function App() {
  ...
  const remove = async (id: string) => { await deleteSupport(id); await refresh(); if (reading?.support.id === id) setReading(null); if (pdfReading?.id === id) setPdfReading(null); if (flashSupport?.id === id) setFlashSupport(null); if (recallSupport?.id === id) setRecallSupport(null); setStatus('Support supprimé.'); };
  ...
  const persistPdfPatch = useCallback((patch: Partial<Support>) => {
    setPdfReading(current => current ? updateStoredSupport(current, patch) : current);
  }, [updateStoredSupport]);
  const savePdfProgress = useCallback((page: number, zoom: number) => {
    setPdfReading(current => {
      ...
      return updateStoredSupport(current, { pdfProgress: { page, zoom, updatedAt: new Date().toISOString() } });
    });
  }, [updateStoredSupport]);
  ...
  const saveFlashcards = useCallback((cards: Flashcard[]) => setFlashSupport(current => current ? updateStoredSupport(current, { flashcards: cards }) : current), [updateStoredSupport]);
  const saveRecallDraft = useCallback((draft: string) => setRecallSupport(current => current ? updateStoredSupport(current, { recallDraft: draft }) : current), [updateStoredSupport]);
  const saveRecallAttempts = useCallback((attempts: RecallAttempt[]) => setRecallSupport(current => current ? updateStoredSupport(current, { recallAttempts: attempts }) : current), [updateStoredSupport]);

  if (recallSupport) return <RecallBoard supportName={recallSupport.name} draft={recallSupport.recallDraft} attempts={recallSupport.recallAttempts} onDraftChange={saveRecallDraft} onAttemptsChange={saveRecallAttempts} onBack={() => setRecallSupport(null)} />;
