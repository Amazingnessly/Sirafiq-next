...
type Support = { id: string; name: string; type: string; size: number; importedAt: string; category?: string; blob?: Blob; dataUrl?: string; extraction?: Extraction; pdfProgress?: PdfProgress; pdfBookmarks?: number[] };
...
function App() {
  ...
  if (pdfReading) return <PdfVisualReader name={pdfReading.name} blob={supportToBlob(pdfReading)} initialPage={pdfReading.pdfProgress?.page} initialZoom={pdfReading.pdfProgress?.zoom} bookmarks={pdfReading.pdfBookmarks} onBack={() => setPdfReading(null)} onProgress={savePdfProgress} onBookmarksChange={savePdfBookmarks} />;
