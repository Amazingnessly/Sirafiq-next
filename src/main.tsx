...
async function normalizeSupportForStorage(support: Support): Promise<Support> {
  if (!(support.blob instanceof Blob)) return support;
  const { blob, ...rest } = support;
  return { ...rest, bytes: await blobToArrayBuffer(blob) };
}

async function saveSupport(support: Support) {
  const db = await openDb();
  const storable = await normalizeSupportForStorage(support);
...
function supportToBlob(support: Support): Blob {
  if (support.bytes instanceof ArrayBuffer) return new Blob([support.bytes], { type: support.type || 'application/octet-stream' });
...
function App() {
  ...
  const remove = async (id: string) => {
    ...
    if (pdfReading?.id === id) setPdfReading(null);
