...
function App() {
    ...
    try {
      ...
      for (const [fileIndex, file] of files.entries()) {
        ...
        try {
          ...
          setStatus(`${files.length > 1 ? `Import ${fileIndex + 1}/${files.length}` : 'Import'} · ${file.name}`);
  ...
  const updateStoredSupport = useCallback((current: Support, patch: SupportPatch) => {
    ...
    void patchSupportMetadata<Support>(current.id, patch)
      .then(saved => {
        setSaveStatus('');
        setSupports(items => items.map(item => item.id === saved.id ? saved : item));
      })
      .catch(() => setSaveStatus('Impossible d’enregistrer les dernières modifications sur cet appareil. Elles restent visibles pour cette session, mais pourraient être perdues en quittant cet écran.'));
    return next;
  }, []);
  ...
  if (recallSupport) {
    return <><RecallBoard supportName={recallSupport.name} draft={recallSupport.recallDraft} attempts={recallSupport.recallAttempts} onDraftChange={saveRecallDraft} onAttemptsChange={saveRecallAttempts} onBack={() => { setSaveStatus(''); setRecallSupport(null); }} /><SaveStatus message={saveStatus} onDismiss={() => setSaveStatus('')} /></>;
  }

  if (flashSupport) {
    return <><Flashcards supportName={flashSupport.name} cards={flashSupport.flashcards ?? []} onChange={saveFlashcards} onBack={() => { setSaveStatus(''); setFlashSupport(null); }} /><SaveStatus message={saveStatus} onDismiss={() => setSaveStatus('')} /></>;
  }

  if (pdfReading) {
    return <><PdfVisualReader name={pdfReading.support.name} source={pdfReading.source} initialPage={pdfReading.support.pdfProgress?.page} initialZoom={pdfReading.support.pdfProgress?.zoom} bookmarks={pdfReading.support.pdfBookmarks} notes={pdfReading.support.pdfNotes} onBack={() => { setSaveStatus(''); setPdfReading(null); }} onProgress={savePdfProgress} onBookmarksChange={savePdfBookmarks} onNotesChange={savePdfNotes} /><SaveStatus message={saveStatus} onDismiss={() => setSaveStatus('')} /></>;
  }
...
if (root) createRoot(root).render(<React.StrictMode><ErrorBoundary area="la bibliothèque et les espaces d’étude"><App /></ErrorBoundary></React.StrictMode>);
